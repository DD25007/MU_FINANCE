...
def _sanitize_probs_array(probs: np.ndarray) -> np.ndarray:
    return np.nan_to_num(
        np.clip(probs, 1e-6, 1.0 - 1e-6), nan=0.5, posinf=1.0, neginf=0.0
    )
...
def compute_auc(
    model: nn.Module, dataset: CreditDataset, device: torch.device
) -> float:
...
def forget_set_accuracy(
    model: nn.Module, forget_ds: CreditDataset, device: torch.device
) -> float:
...
def compute_forget_confidence(
    model: nn.Module, D_forget: CreditDataset, device: torch.device
) -> float:
    probs, _ = get_predictions(model, D_forget, device)
    return float(probs.mean())


def compute_retain_auc(
    model: nn.Module, D_retain: CreditDataset, device: torch.device
) -> float:
    return compute_auc(model, D_retain, device)


def compute_test_auc(
    model: nn.Module, D_test: CreditDataset, device: torch.device
) -> float:
...
def compute_relearn_time(
    ...
    criterion = nn.BCEWithLogitsLoss()
    loader = make_loader(
        D_forget, batch_size=config.get("relearn_batch_size", 64), shuffle=True
    )
    steps = 0
...
def count_updated_params(
    ...
    for (_, p_orig), (_, p_new) in zip(
        ...
        total += p_orig.numel()
        changed += int(
            (~torch.isclose(p_orig.detach().cpu(), p_new.detach().cpu(), atol=atol))
            .sum()
            .item()
        )
...
def full_evaluation(
    ...
    results = {
        ...
        "js_divergence": compute_js_divergence(
            model_star, model_original, test_ds, device
        ),
